...
class ReadOnlyTransaction(APIView):
    def get(self,request,pk=None):
        if pk:
            transaction=get_object_or_404(Transaction,pk=pk)
            serializer=TrasactionReadOnlySerializer(transaction)
            return Response(serializer.data)
        
        transactions=Transaction.objects.all()
        serializer=TrasactionReadOnlySerializer(transactions,many=True)
        return Response(serializer.data)
...
class AccountMoneyMaskedApiView(APIView):
    def get(self,request,pk=None):
        if pk:
            account=get_object_or_404(Account,pk=pk)
...
class TransactionNastedReadAPIView(APIView):
    def get(self,request,pk=None):
        if pk:
            transaction=get_object_or_404(Transaction,pk=pk)
            serializer=TransactionReadNestedSerializer(transaction)
            return Response(serializer.data)
        transactions=Transaction.objects.all()
